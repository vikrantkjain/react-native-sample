...
export default Geocoder = {
   ...
   */
  init(apiKey: string, options: {[key: string]: any}) {
    API_KEY = apiKey;
   ...
   */
  async from(...params: any[]): Promise<object> {
    // check api key
    if (!Geocoder.isInit) {
      throw {
    ...
    if (!isNaN(params[0]) && !isNaN(params[1])) {
      ...
      queryParams = JSON.parse(params[0]);
    }
    ...
    // queryParams.language = Constants.USER_SELECTED_LANGUAGE_CODE;
   ...
   * @deprecated
   */
  getFromLocation(address: string) {
   ...
   * @deprecated
   */
  getFromLatLng(lat: number, lng: number) {
 ...
 */
function toQueryParams(object: {[key: string]: any}): string {
